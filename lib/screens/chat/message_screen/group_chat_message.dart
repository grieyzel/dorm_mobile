...
class _GroupChatMessageScreenState extends State<GroupChatMessageScreen> {
  ...
  Future<void> addUserToGroup(
    ...
    final url = Uri.parse('$BaseUrl/api/group-chats/${widget.chatId}/invite');
    ...
    try {
      ...
      if (response.statusCode == 200) {
        ...
        await fetchGroupMembers(); // optional: refresh group members if needed
        refreshDialog(); // refresh UI
      } else {
  ...
  Future<void> showManageUsersDialog() async {
                      ...
                      ...joinRequests.map((user) {
                        return Padding(
                          ...
                          child: Row(
                            ...
                            children: [
                              buildUserAvatar(context, user), // Avatar

                              SizedBox(width: 8),

                              // Expanded area for name and subtitle
                              Expanded(
                                child: Column(
                                  crossAxisAlignment: CrossAxisAlignment.start,
                                  children: [
  ...
  Future<void> fetchGroupMembers() async {
    ...
    try {
      ...
      if (response.statusCode == 200) {
        ...
        setState(() {
          groupMembers = List<Map<String, dynamic>>.from(json['members']);
          joinRequests = List<Map<String, dynamic>>.from(json['join_requests']);
        });
      }
    } catch (e) {
  ...
  Future<void> respondToJoinRequest(
      String userId, String response, VoidCallback refreshDialog) async {
    final url = Uri.parse('$BaseUrl/api/group-chats/${widget.chatId}/respond');
    ...
    try {
      ...
      if (res.statusCode == 200) {
        final msg = jsonDecode(res.body)['message'] ?? 'Updated';
        toast("✅ $msg");
        await fetchGroupMembers();
        refreshDialog();
      } else {
        ...
        actions: [
          ...
          if (userRole != "S")
            IconButton(
              ...
              onPressed: () async {
                await showManageUsersDialog(); // ✅ await async dialog loader
              },
            ),
        ],
      ),
