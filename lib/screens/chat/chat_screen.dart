...
class _MessageState extends State<Message> {
  ...
  Future<void> fetchUserChats() async {
    final String apiUrl = '$BaseUrl/api/users/$userId/group-chats';
    try {
      ...
      if (response.statusCode == 200) {
        ...
        setState(() {
          groupChats =
              List<Map<String, dynamic>>.from(jsonResponse['joined_groups']);
          pendingGroupInvites =
              List<Map<String, dynamic>>.from(jsonResponse['pending_invites']);
          availableGroups =
              List<Map<String, dynamic>>.from(jsonResponse['available_groups']);
          joinRequestGroups =
              List<Map<String, dynamic>>.from(jsonResponse['join_requests']);
          isLoading = false;
        });
      }
    } catch (e) {
  ...
  List<Map<String, dynamic>> getFilteredChats() {
    ...
    return privateChats.where((chat) {
      ...
      return chatName.toLowerCase().contains(searchQuery.toLowerCase());
    }).toList();
  }
    ...
    return filteredChats.isEmpty
        ...
        : ListView.builder(
            ...
            itemBuilder: (context, index) {
              ...
              final otherUser = chat['user_1'].toString() == userId
                  ? chat['user2']
                  : chat['user1'];
