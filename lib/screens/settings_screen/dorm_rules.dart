...
class _DormRulesScreenState extends State<DormRulesScreen> {
  ...
  Future<void> translateAllRules(String languageCode) async {
    ...
    while (currentSectionStart < entries.length) {
      ...
      // Group all entries in the current section
      ...
      // Translate all entries in the section, one-by-one
      for (final entry in sectionEntries) {
        final translatedKey =
            await translateText(entry.key, languageCode) ?? entry.key;
        final translatedValue =
            await translateText(entry.value, languageCode) ?? entry.value;
